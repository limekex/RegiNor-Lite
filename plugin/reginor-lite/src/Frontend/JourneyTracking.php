...
final class JourneyTracking
{
    ...
    public static function permission(\WP_REST_Request $request): bool|\WP_Error
    {
        ...
        if (!get_option('rnl_journey_enabled', false) || is_user_logged_in() || !function_exists('cmplz_has_consent') || !cmplz_has_consent('statistics')) { return new \WP_Error('rnl_journey_no_consent', __('Målingen krever samtykke til statistikk.', 'reginor-lite'), ['status' => 403]); }
        return true;
    }
    public static function receive(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
    {
        ...
        if (is_array($data) && ($data['operation'] ?? '') === 'forget') {
            ...
            try { $event = JourneyEvent::validate(is_array($data) ? $data : [], function_exists('cmplz_has_consent') && cmplz_has_consent('marketing')); }
